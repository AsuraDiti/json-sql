...
module.exports = function(dialect) {
	...
	dialect.templates.add('select', {
		...
		validate: function(type, params) {
			...
			templateChecks.propType(type, params, 'sort', ['string', 'array', 'object', 'number']);
	...
	dialect.templates.add('create', {
		pattern: 'create {table} {database} {columnlist} {constraintlist}',
		validate: function(type, params) {
			templateChecks.onlyOneOfProps(type, params, ['table', 'database']);
			templateChecks.onlyOneOfProps(type, params, ['database', 'columnlist']);
			templateChecks.onlyOneOfProps(type, params, ['database', 'constraintlist']);

			templateChecks.propType(type, params, 'table', 'string');
			templateChecks.propType(type, params, 'database', 'string');

			templateChecks.propType(type, params, 'columndefinition', ['array', 'object']);
			templateChecks.propType(type, params, 'constraintlist', ['array']);
		}
	});

	dialect.templates.add('columndefinition', {
		pattern: '{name} {type} {primary} {autoincrement} {foreignkey} {unique} {allownull} {default}',
		validate: function(type, params) {
			templateChecks.requiredProp(type, params, 'name');
			templateChecks.requiredProp(type, params, 'type');

			templateChecks.onlyOneOfProps(type, params, ['autoincrement', 'foreignkey']);
			templateChecks.onlyOneOfProps(type, params, ['unique', 'primary']);
			templateChecks.propType(type, params, 'type', ['number', 'object']);
			templateChecks.propType(type, params, 'name', 'string');
			templateChecks.propType(type, params, 'allownull', 'boolean');
			templateChecks.propType(type, params, 'autoincrement', 'boolean');
			templateChecks.propType(type, params, 'primary', 'boolean');
			templateChecks.propType(type, params, 'unique', 'boolean');
			templateChecks.propType(type, params, 'foreignkey', 'object');
			templateChecks.propType(type, params, 'default', ['string', 'number', 'boolean']);
		}
	});

	dialect.templates.add('constraintdefinition', {
		pattern: '{constraintname} {primary} {unique} {foreignkey}',
		validate: function(type, params) {
			templateChecks.onlyOneOfProps(type, params, ['unique', 'primary', 'foreignkey']);

			templateChecks.propType(type, params, 'constraintname', 'string');
			templateChecks.propType(type, params, 'primary', ['string', 'array']);
			templateChecks.propType(type, params, 'unique', ['string', 'array']);
			templateChecks.propType(type, params, 'foreignkey', ['object']);
		}
	});

	dialect.templates.add('drop', {
		pattern: ' drop {table} {database}',
		validate: function(type, params) {
			templateChecks.onlyOneOfProps(type, params, ['table', 'database']);
			templateChecks.propType(type, params, 'table', ['string', 'array']);
			templateChecks.propType(type, params, 'database', ['string', 'array']);
		}
	});
