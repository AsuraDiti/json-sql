...
module.exports = function(dialect) {
	...
	dialect.blocks.add('table', function(params) {
		var prefix = '';
		if(params.type == 'create' || params.type == 'drop'){
			prefix = 'table ';
		}
		if (_.isArray(params.table)){
			return _(params.table).map(function(table) {
				return prefix + dialect.buildBlock('name', {name: table});
			}).join(', ');
		}
		return prefix + dialect.buildBlock('name', {name: params.table});
	});

	dialect.blocks.add('database', function(params) {
		var prefix = '';
		if(params.type == 'create' || params.type == 'drop'){
			prefix = 'database ';
		}
		if (_.isArray(params.database)){
			return _(params.database).map(function(database) {
				return prefix + dialect.buildBlock('name', {name: database});
			}).join(', ');
		}
		return prefix + dialect.buildBlock('name', {name: params.database});
	});
	...
	dialect.blocks.add('type', function(params) {
		return dialect.typemap.get('gettype')(params);
	});

	dialect.blocks.add('allownull', function(params) {
		return params.allownull ? 'null' : 'not null';
	});

	dialect.blocks.add('constrainname', function(params) {
		return ((params.constrainname) ? 'CONSTRAINT ' : '') + dialect.buildBlock('name', {name: params.constrainname});
	});

	dialect.blocks.add('primary', function(params) {
		if(_.isBoolean(params.primary)){
			return params.primary ? 'primary key' : '';
		}else{
			var result = 'primary key ( ';

			result += _(params.primary).map(function(col, fkey) {
				return dialect.buildBlock('name', {name: col});
			}).join(', ');
			result += ' )';
			return result;
		}
	...
	dialect.blocks.add('unique', function(params) {
		if(_.isBoolean(params.unique)){
			return params.unique ? 'unique' : '';
		}else{
			var result = 'unique ( ';

			result += _(params.unique).map(function(col, fkey) {
				return dialect.buildBlock('name', {name: col});
			}).join(', ');
			result += ' )';
			return result;
	...
	dialect.blocks.add('foreignkey', function(params) {
		var result = '';
		var isInColumnList = (_.isUndefined(params.foreignkey.other) || _.isUndefined(params.foreignkey.tablekey));

		var otherKey = (isInColumnList) ? params.foreignkey : params.foreignkey.other;
		var tablekey = (isInColumnList) ? [] : params.foreignkey.tablekey;

		var otherlist = '';
		otherlist = _(otherKey).map(function(columnlist, fkey) {
						var colList = _(columnlist).map(function(col) {
								return dialect.buildBlock('name', {name: col});
							}).join(', ');
						return dialect.buildBlock('name', { name: fkey}) + '(' + colList + ')';
					}).join(', ');

		if(isInColumnList){
			result = 'foreign key references ' + otherlist;
		}else{
			var tableColList = '';
			tableColList = _(tablekey).map(function(col) {

							return dialect.buildBlock('name', {name: col});
						}).join(', ');
			result = 'foreign key ( '+tableColList+' ) references ' + otherlist;
		}
	...
	dialect.blocks.add('autoincrement', function(params) {
		return params.autoincrement ? 'auto_increment' : '';
	});

	dialect.blocks.add('default', function(params) {
		var result = '';
		var oldOption = dialect.builder.options.separatedValues;
		dialect.builder.options.separatedValues = false;

		result = 'default ' + dialect.buildBlock('value', {value: params.default }) ;

		dialect.builder.options.separatedValues = oldOption;

		return result;
	});

	dialect.blocks.add('columnlist', function(params) {
		var colDef = params.columnlist;
		if(_.isObject(colDef) && !_.isArray(colDef))
		{
			colDef = [colDef];
		}

		var result = '( ';
		result += _(colDef).map(function(col) {
						return dialect.buildTemplate('columndefinition', col);
					}).join(', ');
		result += (_.isUndefined(params.constrainlist)) ? ' )':'';
		return result;
	});
	dialect.blocks.add('constrainlist', function(params) {
		var constrains = params.constrainlist;
		if(_.isObject(constrains) && !_.isArray(constrains))
		{
			colDef = [constrains];
		}

		var result = '';
		result += _(constrains).map(function(col) {
						return dialect.buildTemplate('constraindefinition', col);
					}).join(', ');
		result += ' )';
		return result;
	});
